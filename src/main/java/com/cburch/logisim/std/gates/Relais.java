...
public class Relais extends InstanceFactory {
    ...
    static final AttributeOption PORT0IN=new AttributeOption("0",getFromLocale("Port0 Input"));
    static final AttributeOption PORT0OUT= new AttributeOption("1",getFromLocale("Port0 Output"));
    public static final Attribute<AttributeOption> ATTR_DIRECTION=Attributes.forOption("PortInput", 
    		getFromLocale("Select switch direction"),
    		new AttributeOption[]{PORT0IN,PORT0OUT});    
    
	public Relais() {
		super("Relais", getFromLocale("relaisComponent"));
	...
	void computePorts(Instance instance) {
		...
		// ###################Port label##########################################################
		ports[0].setToolTip("port0");
		ports[3].setToolTip("port3");
		ports[4].setToolTip("port4");
	...
	public void propagate(InstanceState state) {
		...
		boolean RelaisActive= ((state.getPort(1) == Value.FALSE)&&(state.getPort(2) == Value.TRUE))||
		((state.getPort(2) == Value.FALSE)&&(state.getPort(1) == Value.TRUE));
		
		if(attrs.getValue(Relais.ATTR_DIRECTION)==PORT0IN){
			state.setData(new RelaisData(RelaisActive,state.getPort(0)));		
			if( RelaisActive ){
				state.setPort(3, Value.NIL, DELAY2);
				state.setPort(4, state.getPort(0), DELAY);
				state.setPort(0, Value.NIL, DELAY2);
			} else{
				state.setPort(4, Value.NIL, DELAY2);
				state.setPort(3, state.getPort(0), DELAY);
				state.setPort(0, Value.NIL, DELAY2);
			}
		} else {
			if( RelaisActive ){
				state.setData(new RelaisData(RelaisActive,state.getPort(4)));		
				state.setPort(3, Value.NIL, DELAY2);
				state.setPort(0, state.getPort(4), DELAY);
				state.setPort(4, Value.NIL, DELAY2);					
			} else{
				state.setData(new RelaisData(RelaisActive,state.getPort(3)));		
				state.setPort(4, Value.NIL, DELAY2);
				state.setPort(0, state.getPort(3), DELAY);
				state.setPort(3, Value.NIL, DELAY2);
			}			
